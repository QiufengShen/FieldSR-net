...
def get_datasets(args, test_seed_offset=0):
    ...
    for n in range(1,7):
        if n != args.cvfold:
            path = '{}/superpoint_graphs/custom_dataset/'.format(args.S3DIS_PATH, n)
    ...
    path = '{}/superpoint_graphs/custom_dataset/'.format(args.S3DIS_PATH, args.cvfold)
...
def get_info(args):
    ...
    return {
        ...
        'classes': 2,
        'inv_class_map': {0:'debris', 1:'terrain'},
    }
...
def preprocess_pointclouds(args):
    ...
    class_count = np.zeros((2,6),dtype='int')
    for n in range(1,7):
        pathP = '{}/parsed/custom_dataset/'.format(S3DIS_PATH, n)
        if args.supervized_partition:
            pathD = '{}/features_supervision/custom_dataset/'.format(S3DIS_PATH, n)
        else:
            pathD = '{}/features/custom_dataset/'.format(S3DIS_PATH, n)
        pathC = '{}/superpoint_graphs/custom_dataset/'.format(S3DIS_PATH, n)
        ...
        for file in os.listdir(pathC):
            ...
            if file.endswith(".h5"):
                ...
                label_count = np.bincount(hard_labels, minlength=2)
